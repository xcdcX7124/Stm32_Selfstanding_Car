...
/***************************************************************************************
  ...
  * 
  * 2026.7.27 已将江科大原适配于Stm32的OLED模组库移植适配到ESP32 S3
  * 同时更新了字库，更新了二维数组以通过编译，删去了中文字库
  * 同时留意SDA和SCL针脚控制函数里的延迟部分，延迟太高会导致通讯缓慢刷新率低，延迟太高会响应不过来通讯失败
  * 一开始使用vtaskdelay(1)，结果刷新率低得感人，改成0以后能使用但刷新率一般，发现这个函数要在Freertos额外处理调度等内容，是通过挂起任务的方式延时的，期间CPU可被其他任务占用
  * 于是改用esp_rom_delay_us，这个函数通过强制占用CPU来延时，延时太久可能会喂不上狗导致复位，但无需处理调度等问题
  * 我认为通讯类部分使用esp_rom_delay_us会更优。现在是2us延迟
  *  ——xcdc
  * 
...
void OLED_W_SCL(uint8_t BitValue)
{
    ...
    gpio_set_level(I2C_Pin_SCL,(uint32_t)BitValue);
	esp_rom_delay_us(2);
...
void OLED_W_SDA(uint8_t BitValue)
{
    ...
    gpio_set_level(I2C_Pin_SDA,(uint32_t)BitValue);
	esp_rom_delay_us(2);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	//...
}
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
    gpio_config_t GPIO_I2CInitStucture={
		.intr_type=GPIO_INTR_DISABLE,
        .mode=GPIO_MODE_INPUT_OUTPUT_OD,
        .pin_bit_mask=(1ULL << I2C_Pin_SCL) | (1ULL << I2C_Pin_SDA),
        .pull_down_en=GPIO_PULLDOWN_DISABLE,
        .pull_up_en=GPIO_PULLUP_ENABLE,
    };
    gpio_config(&GPIO_I2CInitStucture);
